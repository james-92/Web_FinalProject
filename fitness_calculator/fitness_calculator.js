...
function calculate()
{
    ...
    else if(document.getElementById("boy").checked)
    {
        ...
        document.getElementById("type1").textContent= "生活型態偏向座式(例如久坐、臥床、看電視) TEDD= " + type1 + " kcak";
        //type1(BMR*1.2);

        let type2 = BMR *1.375;
        type2 = type2.toFixed(1)//四捨五入到小數點第一位
        document.getElementById("type2").textContent= "從事不太費力的運動(例如散步、開車、烹飪) TEDD= "+ type2 + " kcak";
        //type2(BMR*1.375);

        let type3 = BMR *1.55;
        type3 = type3.toFixed(1)//四捨五入到小數點第一位
        document.getElementById("type3").textContent= "有規律運動的習慣(例如腳踏車、健走、慢跑) TEDD= "+ type3 + " kcak";
        //type3(BMR*1.55);

        let type4 = BMR *1.72;
        type4 = type4.toFixed(1)//四捨五入到小數點第一位
        document.getElementById("type4").textContent= "平時活動程度較高(例如打籃球、游泳、登山) TEDD= "+ type4 + " kcak";
        //type4(BMR*1.72);

        let type5 = BMR *1.9;
        type5 = type5.toFixed(1)//四捨五入到小數點第一位
        document.getElementById("type5").textContent= "會進行高強度的訓練(例如長跑、健身、格鬥) TEDD= "+ type5 + " kcak";
        //type5(BMR*1.9);
        console.log("BMR = " + BMR);
